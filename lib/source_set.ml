...
let list_dates_default ~cwn_data_dir ~ja_dir ~since =
  ...
  | None ->
    ...
    let translated = String.Set.of_list translated_list in
...
let list_dates ~cwn_data_dir ~ja_dir args =
  ...
  let%bind dates =
    ...
    | Some file, _, _ -> validate_file_arg file
    ...
    | None, None, None ->
      let%map dates = list_dates_default ~cwn_data_dir ~ja_dir ~since:args.since in
